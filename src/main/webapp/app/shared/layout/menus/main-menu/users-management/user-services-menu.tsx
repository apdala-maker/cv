...
const userMenuNotAuthenticated = (
  <DropdownMenu>
...
export const UserManagementMenu = ({ isAuthenticated = false }) =>{
  ...
  return(
    <Dropdown isOpen={dropdownOpen} size="sm" className="drop-down-container"  direction="right" toggle={()=>setDropdownOpen(!dropdownOpen)}>
      <DropdownToggle className={dropdownOpen?`sidebar-menu opened`:`sidebar-menu`} caret>
        <span className="float-left">User Management</span>
      </DropdownToggle>
      {isAuthenticated ? userMenuAuthenticated : userMenuNotAuthenticated}
    </Dropdown>
  );
}
