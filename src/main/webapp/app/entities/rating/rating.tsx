  ...
  return (
    <div>
      <h2 id="rating-heading">
        <Translate contentKey="catchControlPanelApp.rating.home.title">Ratings</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="catchControlPanelApp.rating.home.createLabel">Create new Rating</Translate>
        </Link>
      </h2>
        ...
        {ratingList && ratingList.length > 0 ? (
            ...
            <tbody>
              {ratingList.map((rating, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{rating.userCode}</td>
                  <td>{rating.value}</td>
                  <td>{rating.remarks}</td>
                  <td>
                    <TextFormat type="date" value={rating.dateCreated} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td>
                    <TextFormat type="date" value={rating.dateModified} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="catchControlPanelApp.rating.home.notFound">No Ratings found</Translate>
            </div>
          )
        )}
      </div>
