...
export const UserLocation = (props: IUserLocationProps) => {
  ...
  const { userLocationList, match, loading } = props;
  return (
    <div>
      <h2 id="user-location-heading">
        <Translate contentKey="catchControlPanelApp.userLocation.home.title">User Locations</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="catchControlPanelApp.userLocation.home.createLabel">Create new User Location</Translate>
        </Link>
      </h2>
        ...
        {userLocationList && userLocationList.length > 0 ? (
            ...
            <tbody>
              {userLocationList.map((userLocation, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{userLocation.tripCode}</td>
                  <td>{userLocation.userCode}</td>
                  <td>{userLocation.areaCode}</td>
                  <td>
                    <TextFormat type="date" value={userLocation.dateCreated} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td>
                    <TextFormat type="date" value={userLocation.dateModified} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${userLocation.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${userLocation.id}/delete`} color="danger" size="sm">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="catchControlPanelApp.userLocation.home.notFound">No User Locations found</Translate>
            </div>
          )
        )}
      </div>
